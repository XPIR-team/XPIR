...
do_a_test() 
{
	...
	if [[ $VERBOSE == 2 ]]; then
		echo $BASE_DIR/../server/build/PIRServer $S_OPTION
	    echo $BASE_DIR/../client/build/PIRClient -r $PARAM $C_OPTION $@ -c 
	fi
  if [[ $N -le 1000 ]];
  then 
    $BASE_DIR/../server/build/PIRServer $S_OPTION > /tmp/checkpir_server.stdout 2>/tmp/checkpir_server.stderr &
  else
    $BASE_DIR/../server/build/PIRServer $S_OPTION -s $N > /tmp/checkpir_server.stdout 2>/tmp/checkpir_server.stderr &
  fi
  PID=$!
  sleep 1
  $BASE_DIR/../client/build/PIRClient -r $PARAM $C_OPTION $@ -c  > /tmp/checkpir_client.stdout 2> /tmp/checkpir_client.stderr 
}
...
exploit_results()
{
    ...
    if [[ $CORRECT == 1 ]]; then
	    ...
	    if [[ $VERBOSE -ge 1 ]]; then
	      ...
	      echo "Server : $BASE_DIR/../server/build/PIRServer $S_OPTION"
	      echo "Client : $BASE_DIR/../client/build/PIRClient -r $PARAM $C_OPTION $@ -c  "
	      echo "*************** Server stdout **********"
        cat /tmp/checkpir_server.stdout
	      echo "*************** Server stderr **********"
        cat /tmp/checkpir_server.stderr
	      echo "*************** Client stdout **********"
        cat /tmp/checkpir_client.stdout
	      echo "*************** Client stderr **********"
        cat /tmp/checkpir_client.stderr
    ...
    while [[ `ps -ef|grep PIRServer|wc -l` -ne 1 ]] ; do 
      echo -e "Could not kill PIRServer, waiting ..."
      echo -e "\033[2A"
      sleep 1
    done
...
echo -e "##########################################################################"
echo -e "This tool tests that PIRServer and PIRClient run correctly and that an"
...
killall -9 PIRServer >/dev/null 2>/dev/null; sleep 1
