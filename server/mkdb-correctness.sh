...
then
  ...
  dd if=/dev/urandom of=source.random count=`python -c"print($ONE_GBIT / (1024000) );"` bs=128000
fi
...
do
  ...
  rm -fr db
	mkdir db
  dd if=source.random of=db/test1 count=`python -c"print($DB / 1024000 );"` bs=128000
	mv db db-`python -c"print($DB / 8 );"`
done
rm -f source.random
cd ..
