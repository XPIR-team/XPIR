...
bool PIRSession::start(session_option_t session_option)
{
  ...
  else //not driven
  {
    ...
    cryptoMethod = HomomorphicCryptoFactory::getCrypto(fields.at(0));
    cryptoMethod->setNewParameters(pirParam.crypto_params);

    generator = PIRReplyGeneratorFactory::getPIRReplyGenerator(fields.at(0), filesList, pirParam,dbhandler);
...
{
	string buf;
#define NDSS_NO_CATALOG
#ifndef NDSS_NO_CATALOG
    if(dbhandler->getNbStream()>100) {
    ...
    buf = dbhandler->getCatalog(false);
#endif
//	std::cout << "CATALOG = " << buf<<std::endl;
  	// Send the buffer
  	const boost::uint64_t size = buf.size();
//	std::cout << "CATALOG size = " << size<<std::endl;
...
void PIRSession::rcvCryptoParams(bool paramsandkey)
{
  ...
  try{
    ...
    { 
      ...
      cryptoMethod = HomomorphicCryptoFactory::getCrypto(fields[0]); 
      ...
      generator = PIRReplyGeneratorFactory::getPIRReplyGenerator(fields[0], filesList, pirParam, dbhandler);
      if (generator == NULL) 
      {
...
database_t PIRSession::getSavedDatabase()  {
