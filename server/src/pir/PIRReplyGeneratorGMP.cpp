...
PIRReplyGeneratorGMP::PIRReplyGeneratorGMP(vector <std::string>& database_, PIRParameters& param, DBHandler *db) :
...
database_t PIRReplyGeneratorGMP::generateReplyGeneric(bool keep_imported_data = false)
{
  ...
  {
    database_wrapper.database_ptr = (void*)datae;
    database_wrapper.elements_nbr = maxChunkSize;
  } 
  else 
  {
...
void PIRReplyGeneratorGMP::generateReplyGenericFromData(const database_t database)
{
  datae = (mpz_t**) database.database_ptr;
  maxChunkSize = database.elements_nbr;
	boost::mutex::scoped_lock l(mutex);
  //mutex.lock();
  generateReply();
}
