...
PIRReplyGeneratorTrivial::PIRReplyGeneratorTrivial(vector <std::string>& database_, PIRParameters& param, DBHandler *db):
...
database_t PIRReplyGeneratorTrivial::generateReplyGeneric(bool keep_imported_data){
  ...
  if(firstTimeImport)
  {
    ...
    database_wrapper.elements_nbr = totalNbChunks;
...
void PIRReplyGeneratorTrivial::generateReplyGenericFromData(const database_t database)
{
  input_data = (char*) database.database_ptr;
  totalNbChunks = database.elements_nbr;
	boost::mutex::scoped_lock l(mutex);
  generateReply();
}
