...
#include "PIRReplyExtraction.hpp"
#include "../client/PIRViewCLI.hpp"
//#define DEBUG
 ...
 **/
PIRReplyExtraction::PIRReplyExtraction(HomomorphicCrypto& crypto, PIRParameters& param_) :
  filePath("reception"),
  pirParams(param_),
  cryptoMethod(crypto),
 ...
 **/
void PIRReplyExtraction::extractReply(int aggregated_maxFileSize, shared_queue<char*>* clearChunks)
{
  ...
  for (unsigned int rec_lvl = pirParams.d ; rec_lvl >= 1 ; rec_lvl--)
  {
...
#ifdef DEBUG
	std::cout<<"PIRReplyExtraction: First layer ciphertext_nbr="<<ciphertext_nbr<<std::endl;
#endif
...
#ifdef DEBUG
    cout << "PIRReplyExtraction: rec_lvl=" << rec_lvl << " ciphertext_nbr=" << ciphertext_nbr <<  " data_size=" << data_size << " data_size2b=" << data_size2b << endl;
#endif
    cout << "PIRReplyExtraction: Waiting for first replies..." << endl;
    ...
    for (unsigned int j = 0 ; j < ciphertext_nbr ; j++)
    {
      ...
      { 
        cout << "PIRReplyExtraction: Starting reply extraction..." << endl;
      }
...
#ifdef CRYPTO_DEBUG
      cout << "PIRReplyExtraction : pushed " << hex;
  ...
  cout << "PIRReplyExtraction: Reply extraction finished, " << total_ciphertext_nbr <<
 ...
 **/
void PIRReplyExtraction::startExtractReply(int aggregated_maxFileSize, shared_queue<char*>* clearChunks) 
{
  replyThread = boost::thread(&PIRReplyExtraction::extractReply, this, aggregated_maxFileSize, clearChunks);
}

void PIRReplyExtraction::setFileParam(string filename_ ,int fileSize_) 
{
...
PIRReplyExtraction::~PIRReplyExtraction() 
{
  joinThread();
 ...
 **/
void PIRReplyExtraction::joinThread() 
{
  if (replyThread.joinable()) replyThread.join();
}

int PIRReplyExtraction::getChosenFileSize()
{
  return fileSize;
}
