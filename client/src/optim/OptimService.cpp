...
void OptimService::writeHeadFile(unsigned int i, FixedVars& fixedVars)
{
  ...
  file << "# " << std::string("exp" + std::to_string(i)) << " " << tools::getCurrentTime() << std::endl;
...
{  
  ...
  file << std::endl <<"#End " << tools::getCurrentTime() << std::endl;
...
bool OptimService::fileOutdated(std::string crypto_name, std::string extension)
{
  ...
  CryptographicSystem* crypto_ptr = HomomorphicCryptoFactory::getCrypto(crypto_name);
