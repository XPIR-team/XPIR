...
void PIRClientSimple::processCryptoParams() {
  ...
  if (exchange_method == CLIENT_DRIVEN) 
  {
    cryptoMethod = HomomorphicCryptoFactory::getCryptoMethod(optimum.crypto_params);
  ...
  {
    rcvCryptoParams(); 
    cryptoMethod = HomomorphicCryptoFactory::getCryptoMethod(pirParams.crypto_params);
    replyWriter.setCryptoMethod(cryptoMethod);
    sendCryptoParams(!send_paramsandkey);
  }
}
...
{
	PIRQueryGenerator queryGen(pirParams, *cryptoMethod);
 ...
 *  - Stop the programme if any network trouble.
 **/
void PIRClientSimple::uploadWorker(PIRQueryGenerator& queryGen)
{
	unsigned int size = 0;
	char *tmp;
	try 
...
{
	replyExt = new PIRReplyExtraction(*cryptoMethod, pirParams);
 ...
 **/
void PIRClientSimple::downloadWorker(PIRReplyExtraction& turlututu)
{
