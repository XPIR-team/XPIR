...
const unsigned int HomomorphicCryptoFactory::crypto_method_nbr = 2;
const std::vector<std::string>  HomomorphicCryptoFactory::crypto_method_name_vec = [] //use lambda function to initialise the vector
{
...
HomomorphicCrypto* HomomorphicCryptoFactory::getCrypto(std::string cryptoType)
{
  HomomorphicCrypto* h;
  ...
  {
    std::cerr << "HomomorphicCryptoFactory: Warning, unrecognized cryptosystem. Returning NULL" << std::endl;
    h = NULL;
  }
...
HomomorphicCrypto* HomomorphicCryptoFactory::getCryptoMethod(std::string crypto_system_desc)
{
...
void HomomorphicCryptoFactory::getAllCryptoSystems(std::vector<HomomorphicCrypto*>& crypto_sys_vec)
{
...
void HomomorphicCryptoFactory::getOneCryptoSystem(std::vector<HomomorphicCrypto*>& crypto_sys_vec, std::string crypto_system_desc)
{
  std::vector<std::string> fields;
  boost::algorithm::split(fields, crypto_system_desc, boost::algorithm::is_any_of(":"));
